...
```{r}
genotypes<-read.delim("https://raw.githubusercontent.com/sminkler3/BCB546-Spring2022/main/assignments/UNIX_Assignment/fang_et_al_genotypes.txt", header=T)
...
Then I looked at the structure of each file type (genotypes and snps, bur just showing genotypes below)
```{r}
typeof(genotypes)
class(genotypes)
ncol(genotypes)
nrow(genotypes)
```
#Comment on above chunk: original chunk read "genotype", however intended object was "genotypes". I edited the typo and chunk now runs. 
...
```
#Comment on ^: `Group` is not recognized when I run the script. The command works with "Group" instead. 
...
dec_maize_dash <- replace(ord_maize_inc, is.na(ord_maize_inc), "-")
dec_teosin_dash <- replace(ord_teo_inc, is.na(ord_teo_inc), "-")
```
...
```
Covered files to upload to Github. Just showing increasing maize chromosome files below, but was done for all files. 
```{r}
...
## SNPs per chromosome
```{r}
melted_geno<-melt(genotypes, id = c("Sample_ID", "Group"))
...
```
#Comment: Again, I edited "genotype" to "genotypes" and the chunk runs great. 
...
```
#Comment: changed "recoded_geno" to "recoded_genotype"
Graphing
```{r}
...
```
#Comment: I love your graphs! Especially the SNP status by zygosity.
## My Visualization
...
```
#Other comments: Your output files did load to my computer, which is good. However, the maize_inc files seemed to have much less data than the teosinte files. This may not be an issue for this particular assignment but it might be better to have more consistent output files in other projects. So far I haven't found the source of this inconsistency. 
#Additionally, it looks like your positions sorted alphabetically, not numerically, probably because your "Position" column was in character form. 
#Changing the "Position" column to numeric via as.numeric() would probably solve this problem. 
#But overall your script is good! It ran smoothly on my computer and your visualizations are nice. 
